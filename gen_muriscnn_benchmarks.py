...
def get_target_features(target, enable_default=True, enable_muriscvnn=False, enable_cmsisnn=False):
    TARGET_FEATURES = {
        "spike": [
            ...
            *([] if enable_default else []),
            #*([["muriscvnn", "log_instrs"], ["muriscvnn", "vext", "log_instrs"], ["muriscvnn", "pext", "log_instrs"]] if enable_muriscvnn else []),
            *([["muriscvnn", "pext", "log_instrs"]] if enable_muriscvnn else []),
        ...
        "ovpsim": [
            *([] if enable_default else []),
            #*([] if enable_default else []),
            #*([["muriscvnn", "log_instrs"], ["muriscvnn", "vext", "log_instrs"], ["muriscvnn", "pext", "log_instrs"]] if enable_muriscvnn else []),
            *([["muriscvnn", "vext"]] if enable_muriscvnn else []),
            *([["cmsisnn"]] if enable_cmsisnn else []),
        ],
        "host_x86": [
            *([[]] if enable_default else []),
...
VLENS = [32, 64, 128, 256, 512, 1024, 2048]

DEFAULT_VLENS = [64]
#DEFAULT_VLENS = [32, 64, 128, 256, 512, 1024]
                        ...
                        for backend_features in get_backend_features(
                            ...
                            for backend_config in get_backend_config(
                                ...
                                for vlen in vlens:
                                    ...
                                    run.add_features_by_name("log_instrs", context=context)
                                    ...
                                    if args.post:
                                        ...
                                        #run.add_postprocess("analyse_instructions", append=True)
        if args.noop:
            stage = RunStage.LOAD
        else:
            stage = RunStage.RUN
